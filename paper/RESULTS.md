...
provably misses GTP-in-GTP that the robust detector catches), and
`test_reproducibility.py` (the seeded corpus is byte-identical across runs, a
different seed produces a different corpus, and the latency timer covers
dissection rather than starting after it).
...
The timed region spans the whole serialize, bytes, dissect, and evaluate cycle.
This is a correction: the harness previously started its timer after
`IP(bytes(pkt))`, so it reported rule-evaluation cost only. Dissection is about
76% of true per-packet cost on this host, so the earlier "≈206 µs / ≈4,850
pkt/s" and the `README.md`'s older "~8k pkt/s" both overstate what a passive tap
can sustain. Timing rules alone on this host gives ≈166 µs (≈6,000 pkt/s); the
686 µs figure is the one a tap must actually pay. `test_reproducibility.py` now
fails the build if the timer is moved back outside the dissection. Absolute
values are host-dependent; the classification numbers are not.

These absolute values are host-dependent and were produced on the offline
evaluation host. Regenerate with `make eval` before quoting them.
...
|---|---|---|
| R1_GTP_IN_GTP | ✅ 120 | ✅ 22 |
| R2_TEID_SPOOF | ✅ 120 | ✅ 17 |
| R3_CP_SMUGGLING | ✅ 240 | ✅ 37 |
| R4_INNER_TO_CORE | ✅ 360 | ✅ 55 |
